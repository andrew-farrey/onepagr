...
//
// Shares theme.typ and components.typ with onepager_template.typ, same
// explicit theme/theme-grad-passed-as-argument pattern -- see theme.typ's
// header comment for why that (not a wildcard import) is what actually
// makes re-skinning work through shared components. apply-base-styles
// MUST wrap the entire document body including #pagebreak(); see
// components.typ's comment on that function before changing this file's
// structure.
...
// ============================================================
#block(breakable: false, fill: theme-grad.card-bg-grad, stroke: (top: 3pt + theme.brand-blue, rest: 1pt + theme.box-border), inset: 10pt, width: 100%)[
...
#block(breakable: false)[
  #grid(columns: (1fr, 1fr, 1fr, 1fr), column-gutter: 6pt, align: horizon,
    ...
    stroke: (x, ..) => (top: 3pt + theme.brand-blue, rest: 1pt + theme.box-border),
...
#grid(columns: (55fr, 45fr), column-gutter: 16pt,
  ...
  [
    ...
    #rect(fill: theme-grad.callout-bg-grad, stroke: 1pt + theme.box-border, inset: (x: 8pt, y: 5pt), width: 100%)[
      #text(size: 7.5pt, fill: rgb("#1a3a7a"))[Ranked by contribution to the overall shift; see the ranked breakdown below for the full per-category comparison.]
    ]
  ]
)
...
#block(breakable: false)[
  ...
  #grid(columns: (1fr, auto, 1fr, auto, 1fr), column-gutter: 6pt, align: horizon,
    ...
    [#align(center)[#text(size: 1.15em)[*{{tl1_yr}}*] \ #v(2pt) #text(size: 8.5pt)[Window opens; baseline period begins]]],
    ...
    text(size: 20pt, weight: "bold", fill: theme.brand-blue)[#sym.arrow.r],
    [#align(center)[#text(size: 1.15em)[*{{tl3_yr}}*] \ #v(2pt) #text(size: 8.5pt)[Current period; most recent value shown above]]],
  )
]
