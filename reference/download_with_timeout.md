...
– too short for a Quarto release archive (100+ MB) on anything but a
fast connection. Raises it for the duration of this one call only,
always restored afterward (even on error), and converts any download
failure (no connection, DNS failure, a 404 from a bad `version`) into
one clear, onepagr-specific message instead of
